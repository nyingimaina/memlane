...
const JobForm: React.FC<JobFormProps> = ({
  ...
  const [jobCategory, setJobCategory] = useState<JobCategory>(
    initialConfig.dbProvider && initialConfig.dbProvider !== "None"
      ? "Database"
      : "Directory",
  );
  ...
  const handleCategoryChange = (category: JobCategory) => {
    ...
    if (category === "Directory") {
      ...
      const defaultProvider =
        config.dbProvider === "None" ? "SQL Server" : config.dbProvider;
      setConfig({
        ...config,
        dbProvider: defaultProvider,
        dbConnectionString:
          config.dbConnectionString ||
          getTemplateForProvider(defaultProvider || "SQL Server"),
      });
    }
  };
    ...
    >
      ...
      {jobCategory === "Database" && (
        <FormSection title="Database Source" columns={2}>
          <div
            className="database-provider-field"
          ...
          <div className="connection-string-field">
            <ZestTextbox
              ...
              zest={{
                onTextChanged: (val) =>
                  setConfig({ ...config, dbConnectionString: val }),
                stretch: true,
              }}
            />
          </div>
      ...
      <FormSection
        title={
          jobCategory === "Database"
            ? "Source Directory (Optional Sync)"
            : "Source Directory"
        }
